...
('Confirm the list of menu items', async t => {
    ...
    await t.expect(menuItems).eql(['Electron', 'File', 'Edit', 'View', 'Window', 'Help']);
});
...
('Restore a wallet from seed', async t => {
    ...
    console.log(restore_form.child('inputs-item').count);
    ...
    await t
        ...
        .debug()
        .typeText(restore_form.child(4),'password')
        .debug()
        .typeText(restore_form.child(6),'password')
...
('Create a new wallet - confirm incorrect seed', async t => {
    const create_wallet_button = Selector('.welcome-btns')
    .child('div')
    .nth(0);
